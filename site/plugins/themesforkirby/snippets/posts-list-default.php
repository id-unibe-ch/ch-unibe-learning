<div class="max-width-<?= $page->width() ?>">
  ...
  <?php
    ...
    $class = 'full-width post space-bottom';
  ...
  ?>
    ...
    ?>
    <div class="<?= $class ?>">
      <a href="<?php if ($post->externalLink()->isNotEmpty()): ?><?= $post->externalLink() ?><?php else: ?><?= $post->url() ?><?php endif ?>" class="full-width text-decoration-none"<?php if ($post->externalLink()->isNotEmpty()): ?> target="_blank" rel="noopener noreferrer"<?php endif ?>><h2><?= $post->title() ?></h2></a>

      <?php if ($page->author()->bool() && $page->datePublished()->bool()): ?>
        <div class="space-bottom-1x space-top-075x"><span class="display-block font-size-md muted"><?= $post->date()->toDate(dateFormat($site)) ?> — <?php if ($author && $author->name()->isNotEmpty()): ?><?= $author->name() ?><?php else: ?><?= $site->title() ?><?php endif ?></span></div>
      <?php elseif ($page->author()->bool()): ?>
        <div class="space-bottom-1x space-top-075x"><span class="display-block font-size-md muted"><?php if ($author && $author->name()->isNotEmpty()): ?><?= $author->name() ?><?php else: ?><?= $site->title() ?><?php endif ?></span></div>
      <?php elseif ($page->datePublished()->bool()): ?>
        <div class="space-bottom-1x space-top-075x"><span class="display-block font-size-md muted"><?= $post->date()->toDate(dateFormat($site)) ?></span></div>
      <?php endif ?>

      <?php if ($page->media()->bool() && $cover = $post->cover()->toFile()): ?>

        <?php if ($page->mediaFixedHeight()->bool() === false): ?>
          <a href="<?php if ($post->externalLink()->isNotEmpty()): ?><?= $post->externalLink() ?><?php else: ?><?= $post->url() ?><?php endif ?>" class="<?php if ($page->mediaBorder()->bool()): ?>border border-img <?php endif ?>full-width position-relative<?php if ($page->mediaRounded()->bool()): ?> rounded<?php endif ?><?php if ($page->mediaShadow()->value() !== 'none'): ?> shadow-<?= $page->mediaShadow() ?><?php endif ?> space-bottom-1x space-top-1x text-decoration-none"<?php if ($post->externalLink()->isNotEmpty()): ?> target="_blank" rel="noopener noreferrer"<?php endif ?>>
        ...
        <?php else: ?>
          <a href="<?php if ($post->externalLink()->isNotEmpty()): ?><?= $post->externalLink() ?><?php else: ?><?= $post->url() ?><?php endif ?>" class="align-items-<?= $page->mediaPositionVertical() ?> <?php if ($page->mediaBorder()->bool()): ?>border border-img <?php endif ?>full-width max-height-md position-relative<?php if ($page->mediaRounded()->bool()): ?> rounded<?php endif ?><?php if ($page->mediaShadow()->value() !== 'none'): ?> shadow-<?= $page->mediaShadow() ?><?php endif ?> space-bottom-2x space-top-2x text-decoration-none"<?php if ($post->externalLink()->isNotEmpty()): ?> target="_blank" rel="noopener noreferrer"<?php endif ?>>
      ...
      <?php if ($post->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-1x"><?= $post->text()->excerpt(200) ?></p>
      <?php elseif ($post->heroText()->isNotEmpty()): ?>
        <p class="paragraph space-top-1x"><?= $post->heroText()->excerpt(200) ?></p>
      <?php elseif ($blockFirst && $blockFirst->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-1x"><?= $blockFirst->text()->excerpt(200) ?></p>
      <?php elseif ($featureFirst && $featureFirstBlockFirst && $featureFirstBlockFirst->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-1x"><?= $featureFirstBlockFirst->text()->excerpt(200) ?></p>
      <?php endif ?>
