<div class="row row-fill-empty-columns row-gutter-lg<?php if (!isset($related)): ?> max-width-<?= $page->width() ?><?php endif ?>">

  <?php

    // Classes

    $class = 'col-1-' . $page->gridColumns();
  ...
  ?>
    <div class="<?= $class ?>">
      ...
      <?php if ($page->media()->bool() && $cover = $post->cover()->toFile()): ?>
        ...
        <?php if (!isset($related) && $page->mediaFixedHeight()->bool() === false): ?>
          <div class="<?php if ($page->mediaBorder()->bool()): ?>border border-img <?php endif ?>full-width<?php if ($page->mediaRounded()->bool()): ?> rounded<?php endif ?><?php if ($page->mediaShadow()->value() !== 'none'): ?> shadow-<?= $page->mediaShadow() ?><?php endif ?> space-bottom-15x">
      ...
      <?php if ($post->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-025x"><?php if ($page->gridColumns()->value() === '3'): ?><?= $post->text()->excerpt(80) ?><?php else: ?><?= $post->text()->excerpt(160) ?><?php endif ?></p>
      <?php elseif ($post->heroText()->isNotEmpty()): ?>
        <p class="paragraph space-top-025x"><?php if ($page->gridColumns()->value() === '3'): ?><?= $post->heroText()->excerpt(80) ?><?php else: ?><?= $post->heroText()->excerpt(160) ?><?php endif ?></p>
      <?php elseif ($blockFirst && $blockFirst->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-025x"><?php if ($page->gridColumns()->value() === '3'): ?><?= $blockFirst->text()->excerpt(80) ?><?php else: ?><?= $blockFirst->text()->excerpt(160) ?><?php endif ?></p>
      <?php elseif ($featureFirst && $featureFirstBlockFirst && $featureFirstBlockFirst->text()->isNotEmpty()): ?>
        <p class="paragraph space-top-025x"><?php if ($page->gridColumns()->value() === '3'): ?><?= $featureFirstBlockFirst->text()->excerpt(80) ?><?php else: ?><?= $featureFirstBlockFirst->text()->excerpt(160) ?><?php endif ?></p>
      <?php endif ?>
