<?php $numFeature = 0; foreach ($page->features()->toStructure() as $feature): $numFeature = ++$numFeature; ?>
...
.feature-<?= $numFeature ?> .bg-color-custom.block-<?= $numBlock ?> {
  background: #<?= $backgroundBlockColor->custom() ?>;
}
<?php endif ?>
<?php if ($backgroundBlockColor->customDark()->isNotEmpty()): ?>
body.dark .feature-<?= $numFeature ?> .bg-color-custom.block-<?= $numBlock ?> {
...
.feature-<?= $numFeature ?> .bg-gradient-custom.block-<?= $numBlock ?> {
...
body.dark .feature-<?= $numFeature ?> .bg-gradient-custom.block-<?= $numBlock ?> {
