...
<?php snippet('header') ?>
  ...
  <?php
    ...
    if ($backgroundPage && $backgroundPageClass) {
      $class = $backgroundPageClass . ' page-' . $page->slug() . ' posts';
    } else {
      $class = 'page-' . $page->slug() . ' posts';
    }
  ...
  <main class="<?= $class ?>">
    ...
    <?php elseif ($page->hasListedChildren()): ?>
      ...
      ?>
      <section id="posts-<?= $page->slug() ?>" class="<?= $class ?>">

        <?php

          // Classes

          $class = 'align-' . $page->align() . ' posts-' . $page->layout();
